...
static const struct sensor_t sSensorList[] = {
    ...
    },
    {
        ...
        /*	magnetic field sensor and orientation sensor	*/
    {
        .name = "BOSCH BMM050 Magnetic Field Sensor",
    ...
    {
        .name = "BOSCH BMM050 Orientation Sensor",
        .vendor = "Bosch Sensortec, GmbH",
        .version = 1,
	...
	/* Accelerometer */
    {
        .name = "BOSCH BMA250 Acceleration Sensor",
        .vendor = "Bosch Sensortec, GmbH",
        .version = 1,
...
sensors_poll_context_t::sensors_poll_context_t()
{
	...
	mPollFds[bst].fd = open(FIFO_DAT,O_RDWR);
	mPollFds[bst].events = POLLIN;
	mPollFds[bst].revents = 0;

	mSensors[bst_acc] = new BstSensorAccel(DEV_NAME_A, TOP_Y_FORWARD);
...
int sensors_poll_context_t::activate(int handle, int enabled) {
	...
	ALOGD("activate, handle: %d, enabled: %d",
			handle, enabled);
...
int sensors_poll_context_t::setDelay(int handle, int64_t ns) {
	...
	ALOGD("setDelay, handle: %d delay: %jd",
			handle, ns);
...
const struct sensor_t * BstSensorInfo::getSensor(int handle)
{
	...
	for (i = 0; i < ARRAY_SIZE(sSensorList); i++) {
		if (sSensorList[i].handle == handle) {
			break;
		}
	}

	if (i < ARRAY_SIZE(sSensorList)) {
		s = (const struct sensor_t *) sSensorList + i;
	}
